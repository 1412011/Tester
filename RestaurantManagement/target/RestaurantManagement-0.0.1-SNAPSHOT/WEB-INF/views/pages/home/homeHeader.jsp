...
<div class="gradient fm-main-header ">
					...
					<div class="bar">
						<div class="view-menu">
							...
							<c:if test="${branchAddress != null}">
								<div class="arrow">
									<span id="branchAddress">${branchAddress }</span>
								</div>
							</c:if>
						</div>
					</div>
	...
	<c:if test="${not empty dishCategoryList }">
		<div id="fm-collection-nav" class="fm-collection-nav">
			<ul id="ulDishCategory">
				<c:forEach var="dishCategory" items="${dishCategoryList }">
					<li class="liDishCategoryItem" data-href="#${dishCategory.id }"
						id="fm-nav-id-1" class=""><a href="#${dishCategory.id }"
						class="aDishCategoryItem">${dishCategory.name }</a></li>
				</c:forEach>

			</ul>
		</div>
	</c:if>
	<div id="" class="fm-collection-nav">
		<div id="divSearchDish">
			<input id=ipSearchDish type="text" placeholder="Search dishes..."
				data-id="">

			<button id="btSearchDish" type="button">
